...
void Convert_CSV_to_ROOT() {
    ...
    vector<string> label_ptbias_array   = {"B8_Flat", "B8", "B4", "B0"};
...
//        {{"Train_Centered_Test_40_60","40","60"}, {"10","40"}, {"20","50"}, {"30","60"}, {"40","70"}, {"50","80"}, {"60","90"}}
    ...
    for ( int i=0 ; i < label_ptbias_array.size() ; i++ ){
        string dir_train = dir_master + "Train_" + label_ptbias_array[i] + "/";
        ...
        string plot_csv_data_path = dir_plot_csv + "Train_" + label_ptbias_array[i] + "_Plot_Info.csv";
        plot_csv_data.open(plot_csv_data_path.c_str());
        plot_csv_data << "root_file_path, train_ptbias, train_features, train_test_type, train_pt_min, train_pt_max, test_pt_min, test_pt_max, lr_coeffs_tree, rf_coeffs_tree, lr_results_tree, rf_results_tree, mlp_results_tree, plot_delta, plot_lr_coeffs, plot_comparison,\n";
        ...
        for ( int j=0 ; j < label_features_array.size() ; j++ ){
            ...
            for ( int k=0 ; k < test_bin_array.size() ; k++ ){
                ...
                string root_name = "Train_" + label_ptbias_array[i] + "_" + label_features_array[j] + "_" + label_ptminmax[0] + "_" + label_ptminmax[1];
                ...
                for ( int l=0 ; l < label_coeffs_array.size() ; l++ ){
                    string csv_coeff_path   = dir_train + root_name + "_" + label_coeffs_array[l] + ".csv";
                ...
                for ( int l=1 ; l < test_bin_array[k].size() ; l++ ){
                    ...
                    for (  int m=0 ; m < label_mltype_array.size() ; m++ ){
                        ...
                        string csv_path  = dir_csv + "Train_" + label_ptbias_array[i] + "_" + label_features_array[j] + "_" + label_ptminmax[0] + "_" + label_ptminmax[1] + "_" + tree_name + ".csv";
                    ...
                    plot_csv_data << (label_ptbias_array[i] + ",").c_str();
            ...
            for ( int k=0 ; k < traintest_bin_array.size() ; k++ ){
                ...
                string root_path = dir_root + "Train_" + label_ptbias_array[i] + "_" + label_features_array[j] + "_" + label_ptminmax[0] + "_" + label_ptminmax[1] + "_" + traintest_bin_array[k][0][0] + ".root";
                std::cout << "ROOT File Path: " << root_path.c_str() << std::endl;
                for ( int l=1 ; l < traintest_bin_array[k].size() ; l++ ){
                    ...
                    string csv_base = "Train_" + label_ptbias_array[i] + "_" + label_features_array[j] + "_" + traintest_bin_array[k][l][0] + "_" + traintest_bin_array[k][l][1];
...
//    Convert_CSV_to_ROOT();
    ...
    // Output plots from ROOT file
    vector<string> plot_file_array = {
        "../../Files/Thesis_Data/CSV_Plot_Info/Train_B8_Flat_Plot_Info.csv",
        "../../Files/Thesis_Data/CSV_Plot_Info/Train_B8_Plot_Info.csv",
        "../../Files/Thesis_Data/CSV_Plot_Info/Train_B4_Plot_Info.csv",
        "../../Files/Thesis_Data/CSV_Plot_Info/Train_B0_Plot_Info.csv"
    };
    
    for ( int i=0 ; i < plot_file_array.size() ; i++ ) {
        ifstream plot_csv_file;
        plot_csv_file.open(plot_file_array[i], ios::in);
        ...
        while (!plot_csv_file.eof()) {
            ...
            string label_feature    = plot_data[2];
            string label_test_type  = plot_data[3];
            string train_pt_min     = plot_data[4];
            string train_pt_max     = plot_data[5];
            string test_pt_min      = plot_data[6];
            string test_pt_max      = plot_data[7];
            string lr_coeffs_tree   = plot_data[8];
            string rf_coeffs_tree   = plot_data[9];
            string lr_results_tree  = plot_data[10];
            string rf_results_tree  = plot_data[11];
            string mlp_results_tree = plot_data[12];
            bool   plot_delta       = false;
            bool   plot_lr_coeffs   = false;
            bool   plot_comparison  = false;
            if ( plot_data[13].compare("true") == 0 ) plot_delta      = true;
            if ( plot_data[14].compare("true") == 0 ) plot_lr_coeffs  = true;
            if ( plot_data[15].compare("true") == 0 ) plot_comparison = true;
            ...
            string dir_plot_type = dir_plots + "Train_" + label_ptbias + "_" + label_feature + "_" + label_test_type + "/";
            std::__fs::filesystem::create_directories(dir_plot_type.c_str());
            
            if (false){ //( plot_delta ) {
                string plot_delta_title_base = label_ptbias + "_" + label_feature + "_" + label_test_type + "_Train_" + train_pt_min + "_" + train_pt_max + "_Test_" + test_pt_min + "_" + test_pt_max;
            ...
            if ( plot_comparison  ) {
                string plot_comparison_title = label_ptbias + "_" + label_feature + "_" + label_test_type + "_Train_" + train_pt_min + "_" + train_pt_max + "_Test_" + test_pt_min + "_" + test_pt_max;
                Plot_Comparison(
                    root_path,
                    dir_plot_type,
